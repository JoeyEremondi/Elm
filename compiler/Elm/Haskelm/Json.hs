...
mkToJSON :: Options -- ^ Encoding options.
         -> Dec -- ^ Name of the type to encode.
         -> Q Exp
mkToJSON opts dec = withDec dec (\_ cons -> consToJSON opts cons)
...
mkParseJSON :: Options -- ^ Encoding options.
            -> Dec -- ^ Name of the encoded type.
            -> Q Exp
mkParseJSON opts dec =
    withDec dec (\_ cons -> consFromJSON name opts cons)
    where (DataD _ name _ _ _) = dec
