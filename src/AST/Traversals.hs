...
mapMExpr fm (A.A ann e) =
  do
    newExpr <- newExprM
    newAnn <- return ann --TODO remove
    fm $ A.A newAnn newExpr
  where
    self = mapMExpr fm
    newExprM =
      case e of
        (Literal _) -> return e
        (Var _) -> return e
        (Range start end) -> Range <$> self start <*> self end 
        (ExplicitList subList) -> ExplicitList <$> forM subList self
        (Binop op arg1 arg2) -> (Binop op) <$> self arg1 <*> self arg2
        (Lambda pat body) -> (Lambda pat) <$> self body
        (App fn arg) -> App <$> self fn <*> self arg
        (MultiIf pairs) -> do
          let (conds, exprs) = unzip pairs
          newConds <- forM conds self
          newExprs <- forM exprs self
          return $ MultiIf $ zip newConds newExprs
        (Let defs body) -> do
          newDefs <- forM defs $ mapMDef fm
          newBody <- self body
          return $ Let newDefs newBody
        (Case cexp branches) -> do
          let (pats, branchExps) = unzip branches
          newCexp <- self cexp
          newExps <- forM branchExps self
          return $ Case newCexp $ zip pats newExps
        (Data ctor args) -> (Data ctor) <$> forM args self
        (Access recExp field) -> Access <$> self recExp <*> return field
        (Remove recExp field) -> Remove <$> self recExp <*> return field
        (Insert recExp field arg) -> Insert <$> self recExp <*> return field <*> self arg
        (Modify recExp fieldPairs) -> do
          let (names, vals) = unzip fieldPairs
        ...
        (Port sub) -> Port <$> mapMPort fm sub
        (GLShader _ _ _) -> return e
...
mapExpr f e =
  let
    fm x = return $ f x
  in MI.runIdentity $ mapMExpr fm e
